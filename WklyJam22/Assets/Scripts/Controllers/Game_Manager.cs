...
public class Game_Manager : MonoBehaviour {
	...
	/* 	else if (SceneManager.GetActiveScene().buildIndex == 0){
			...
			buttonHolder.transform.GetChild(0).gameObject.GetComponentInChildren<Button>().onClick.RemoveAllListeners();
			buttonHolder.transform.GetChild(0).gameObject.GetComponentInChildren<Button>().onClick.AddListener(() => StartGame());
			buttonHolder.transform.GetChild(1).gameObject.GetComponentInChildren<Button>().onClick.RemoveAllListeners();
